...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('modal_remove_license_')) {
        ...
        const script = decodeURIComponent(parts.slice(4).join('_'));
        ...
        licenses[userId] = (licenses[userId] || []).filter(l => l.script !== script);
        ...
        const reply = originalLength === newLength
            ? `Licença **${script}** não encontrada para <@${userId}>.`
            : `Licença **${script}** removida de <@${userId}>.\n📝 Motivo: ${reason}`;
