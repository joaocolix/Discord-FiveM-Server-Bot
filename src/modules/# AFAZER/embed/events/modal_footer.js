...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isModalSubmit() && interaction.customId.startsWith("modal_footer_")) {
        const index = parseInt(interaction.customId.split("_")[2]);
    
        const texto = interaction.fields.getTextInputValue("texto") || null;
        const iconURL = interaction.fields.getTextInputValue("icone") || null;
