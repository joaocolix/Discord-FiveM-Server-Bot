...
const roleId = '1358117025205190908'; // ID do cargo que deseja atribuir quando o usuário iniciar a live
const guildId = '1335799500194844692'; // ID do servidor Discord onde deseja aplicar as ações
const channelId = '1358116262684786998'; // ID do canal onde o bot enviará a mensagem da live
            ...
            if (response.data.data.length > 0) {
                ...
                if (stream.type === 'live' && !user.live) {
                    ...
                    if (!messageSent) {
                        ...
                        await channel.send(`${member} está ao vivo!\nAssista em: https://www.twitch.tv/${user.username}`)
                            .catch(error => console.error(`Erro ao enviar mensagem: ${error.message}`));
                    }
                }
            } else {
                const wasLive = await db.get(`twitchUser_${user.username}.live`);
                if (wasLive) {
                    await db.set(`twitchUser_${user.username}.live`, false);
                    await db.set(`twitchUser_${user.username}.messageSent`, false);
