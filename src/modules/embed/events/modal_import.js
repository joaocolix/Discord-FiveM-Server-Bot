...
client.on('interactionCreate', async (interaction) => {
    if (interaction.isModalSubmit() && interaction.customId.startsWith("modal_import_")) {
        const index = parseInt(interaction.customId.split("_")[2]);
        const jsonText = interaction.fields.getTextInputValue("json_embed");
        ...
        try {
            const newEmbed = Discord.EmbedBuilder.from(jsonData);
            embeds[index] = newEmbed;
